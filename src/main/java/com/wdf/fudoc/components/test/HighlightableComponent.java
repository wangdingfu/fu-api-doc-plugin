    ...
    protected void paintComponent(Graphics g) {
        ...
        if (myHighlightedRegions.size() == 0) {
            ...
            for (HighlightedRegion hRegion : myHighlightedRegions) {
                ...
                //只针对当前鼠标所在的链接特殊处理
                if (isCurrent && hRegion.textAttributes.getEffectType() != null && hRegion.textAttributes.getEffectColor() != null) {
                    g.setColor(hRegion.textAttributes.getEffectColor());
                    int y = yOffset/*fontMetrics.getMaxAscent()*/ + 2;
                    LinePainter2D.paint((Graphics2D) g, offset + 5, y, offset + fontMetrics.stringWidth(text) - 6, y);
                }
